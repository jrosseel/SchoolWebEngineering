...
alter table `request`   add index fk_request_notification_2 (`notification_oid`), add constraint fk_request_notification_2 foreign key (`notification_oid`) references `notification` (`oid`);


-- Notification_Rejected_Request [rel3]
alter table `request`  add column  `notification_oid_2`  integer;
alter table `request`   add index fk_request_notification (`notification_oid_2`), add constraint fk_request_notification foreign key (`notification_oid_2`) references `notification` (`oid`);


-- Address_UserProfile [rel4]
alter table `userprofile`  add column  `address_oid`  integer;
alter table `userprofile`   add index fk_userprofile_address (`address_oid`), add constraint fk_userprofile_address foreign key (`address_oid`) references `address` (`oid`);
