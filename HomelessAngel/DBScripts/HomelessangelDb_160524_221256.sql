...
create table `userprofile` (
   `userprofileoid`  integer  not null,
   `birthdate`  date,
   `isdisabled`  bit,
   `disabledreason`  bit,
   `iscancelled`  bit,
  ...
  primary key (`userprofileoid`)
);


-- Angel [ent3]
create table `angel` (
   `userprofile_userprofileoid`  integer  not null,
   `angeloid`  integer  not null,
   `profession`  varchar(255),
   `isanomynous`  bit,
  primary key (`userprofile_userprofileoid`)
);
...
create table `homelessperson` (
   `userprofile_userprofileoid`  integer  not null,
   `homelesspersonoid`  integer  not null,
...
alter table `user_group`   add index fk_user_group_user (`user_oid`), add constraint fk_user_group_user foreign key (`user_oid`) references `user` (`useroid`);
...
alter table `request`   add index fk_request_homelessperson (`homelessperson_oid`), add constraint fk_request_homelessperson foreign key (`homelessperson_oid`) references `homelessperson` (`userprofile_userprofileoid`);


-- UserProfile_Notification [rel13]
alter table `notification`  add column  `userprofile_oid`  integer;
alter table `notification`   add index fk_notification_userprofile (`userprofile_oid`), add constraint fk_notification_userprofile foreign key (`userprofile_oid`) references `userprofile` (`userprofileoid`);
...
alter table `picture`   add index fk_picture_userprofile (`userprofile_oid`), add constraint fk_picture_userprofile foreign key (`userprofile_oid`) references `userprofile` (`userprofileoid`);
...
alter table `userprofile`   add index fk_userprofile_user (`user_oid`), add constraint fk_userprofile_user foreign key (`user_oid`) references `user` (`useroid`);
...
alter table `offer`   add index fk_offer_angel (`angel_oid`), add constraint fk_offer_angel foreign key (`angel_oid`) references `angel` (`userprofile_userprofileoid`);


-- GEN FK: Angel --> UserProfile
alter table `angel`   add index fk_angel_userprofile (`userprofile_userprofileoid`), add constraint fk_angel_userprofile foreign key (`userprofile_userprofileoid`) references `userprofile` (`userprofileoid`);


-- GEN FK: HomelessPerson --> UserProfile
alter table `homelessperson`   add index fk_homelessperson_userprofile (`userprofile_userprofileoid`), add constraint fk_homelessperson_userprofile foreign key (`userprofile_userprofileoid`) references `userprofile` (`userprofileoid`);


-- Angel.avgRating [ent3#att1]
create view `angel_avgrating_view` as
select AL1.`userprofile_userprofileoid` as `userprofile_userprofileoid`, avg(AL5.`score`) as `der_attr`
from  `angel` AL1 
               left outer join `offer` AL2 on AL1.`userprofile_userprofileoid`=AL2.`angel_oid`
...
group by AL1.`userprofile_userprofileoid`;


-- Angel.amtOfItemsGivenAway [ent3#att15]
create view `angel_amtofitemsgivenaway_view` as
select AL1.`userprofile_userprofileoid` as `userprofile_userprofileoid`, sum(AL3.`amount`) as `der_attr`
from  `angel` AL1 
               left outer join `offer` AL2 on AL1.`userprofile_userprofileoid`=AL2.`angel_oid`
               left outer join `request` AL3 on AL2.`oid`=AL3.`offer_oid`
